...
int main(int argc, char *argv[])
{
	...
	double distancia_no_sqrt, pendiente, angulo, temp_fuer, temp_fuer_x, temp_fuer_y, j_pos_x, j_pos_y, j_mas, k_pos_x, k_pos_y, k_mas;
	for (int i = 0; i < num_iteraciones; ++i)
	{
		for (unsigned int j = 0; j < vec_ast.size(); ++j) // Iterador principal de Asteroides
		{
			// Se extraen los datos del asteroide j
			j_pos_x = vec_ast[j].get_pos_x();	// Asteroide j, coordenada x
			j_pos_y = vec_ast[j].get_pos_y();	// Asteroide j, coordenada y
			j_mas	= vec_ast[j].get_m();		// Asteroide j, masa
			...
			for (unsigned int k = 0; k < vec_ast.size(); ++k) // Iterador secundario de Asteroides
			{
				...
				double par_k_pos_x	= vec_ast[k].get_pos_x();	// Asteroide k, coordenada x
				double par_k_pos_y	= vec_ast[k].get_pos_y();	// Asteroide k, coordenada y
				double par_k_mas	= vec_ast[k].get_m();		// Asteroide k, masa
				...
				double par_distancia_no_sqrt = ((j_pos_x - par_k_pos_x) * (j_pos_x - par_k_pos_x)) + ((j_pos_y - par_k_pos_y) * (j_pos_y - par_k_pos_y));
				double par_distancia_real = sqrt(par_distancia_no_sqrt);

				if (par_distancia_real > dmin)
				{
					double par_pendiente = (par_k_pos_y - j_pos_y) / (par_k_pos_x - j_pos_x); // Cálculo de la pendiente
					if (!isinf(par_pendiente) && (par_pendiente > 1 || par_pendiente < -1))
					{
						par_pendiente -= trunc(par_pendiente); // Truncado de la pendiente
					}
					double par_angulo = atan(par_pendiente); // Cálculo del ángulo de efecto

					double par_temp_fuer = gravity * j_mas * par_k_mas / par_distancia_no_sqrt; // Fuerza total
					if (par_temp_fuer > 200)
					{
						par_temp_fuer = 200; // Truncado de la fuerza
					}

					double par_temp_fuer_x = par_temp_fuer * cos(par_angulo); // Componente x de la fuerza total
					double par_temp_fuer_y = par_temp_fuer * sin(par_angulo); // Componente y de la fuerza total

					vec_ast[k].set_fue_x(vec_ast[k].get_fue_x() + par_temp_fuer_x); // Al asteroide k se le resta
					vec_ast[k].set_fue_y(vec_ast[k].get_fue_y() + par_temp_fuer_y);
				}
			}
			...
			for (unsigned int k = 0; k < vec_pla.size(); ++k) // Iterador de Planetas
			{
				...
				k_pos_x = vec_pla[k].get_pos_x();
				k_pos_y = vec_pla[k].get_pos_y();
				k_mas = vec_pla[k].get_m();

				distancia_no_sqrt = ((j_pos_x - k_pos_x) * (j_pos_x - k_pos_x)) + ((j_pos_y - k_pos_y) * (j_pos_y - k_pos_y));

				pendiente = (j_pos_y - k_pos_y) / (j_pos_x - k_pos_x);
				...
				temp_fuer = gravity * j_mas * k_mas / distancia_no_sqrt;
				if (temp_fuer > 200)
				{
					temp_fuer = 200;
				}

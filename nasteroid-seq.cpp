...
int main(int argc, char *argv[])
{
	...
	double distancia_no_sqrt, distancia_real, pendiente, angulo, temp_fuer, temp_fuer_x, temp_fuer_y, j_pos_x, j_pos_y, j_mas, k_pos_x, k_pos_y, k_mas;
	for (int i = 0; i < num_iteraciones; ++i)
	{
		for (unsigned int j = 0; j < vec_ast.size(); ++j) // Iterador principal de Asteroides
		{
			...
			for (unsigned int k = j+1; k < vec_ast.size(); ++k) // Iterador secundario de Asteroides
			{
				// Se extraen los datos del asteroide k
				k_pos_x = vec_ast[k].get_pos_x();	// Asteroide k, coordenada x
				k_pos_y = vec_ast[k].get_pos_y();	// Asteroide k, coordenada y
				k_mas	= vec_ast[k].get_m();		// Asteroide k, masa
				...
				{
					pendiente = (j_pos_y - k_pos_y) / (j_pos_x - k_pos_x); // Cálculo de la pendiente
					...
					temp_fuer = gravity * j_mas * k_mas / distancia_no_sqrt; // Fuerza total
					if (temp_fuer > 200)
					{
